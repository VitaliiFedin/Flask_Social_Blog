...
class User(UserMixin, db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if self.email is not None and self.avatar_hash is None or self.avatar_hash is 'null':
    ...
    def generate_confirmation_token(self, expiration=3600):
        s = Serializer(bytes(current_app.config['SECRET_KEY'], 'utf-8'))
    ...
    def generate_reset_email_token(self, new_email):
        s = Serializer(bytes(current_app.config['SECRET_KEY'], 'utf-8'))
...
class Comment(db.Model):
    ...
    def from_json(json_comment):
        ...
        if body is None or body == '':
            raise ValidationError('Comment doesn\'t have body')
